...
export class AdminFlow {
    ...
    async hardDeleteUser(req: HardDeleteUserRequest): Promise<void> {
     ...
     * caller isn't admin.
     */
    async getUsersBulk(req: LookupUsersRequest): Promise<LookupUserRecord[]> {
        const resp = await this.deps.ports.transport.request<{ users: LookupUserRecord[] }>({
            method: 'POST',
     ...
     */
    async listUsers(req: ListUsersRequest = {}): Promise<ListUsersResult> {
        const params = new URLSearchParams();
        if (req.search) params.set('search', req.search);
        if (req.page) params.set('page', String(req.page));
        if (req.pageSize) params.set('page_size', String(req.pageSize));
        if (req.organizationId) params.set('organization_id', req.organizationId);
        if (req.appId) params.set('app_id', req.appId);
        const qs = params.toString();
        const resp = await this.deps.ports.transport.request<ListUsersResult>({
            method: 'GET',
    ...
    async listUserRoles(userId: string): Promise<UserRoleRecord[]> {
        const resp = await this.deps.ports.transport.request<{ roles: UserRoleRecord[] }>({
            method: 'GET',
