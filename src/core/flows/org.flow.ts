...
export class OrgFlow {
    ...
    async get(orgId: string): Promise<Organization> {
        const resp = await this.deps.ports.transport.request<Organization>({
            method: 'GET',
    ...
    async adminListMembers(orgId: string): Promise<OrgMemberRecord[]> {
        const resp = await this.deps.ports.transport.request<{ members: OrgMemberRecord[] }>({
            method: 'GET',
     ...
     */
    async adminGetUserOrganizations(userId: string): Promise<AdminUserOrgMembership[]> {
        const resp = await this.deps.ports.transport.request<{ memberships: AdminUserOrgMembership[] }>({
            method: 'GET',
     ...
     */
    async adminAddMember(orgId: string, userId: string, roleIds: string[] = []): Promise<void> {
        const resp = await this.deps.ports.transport.request({
            method: 'POST',
            url: `${this.deps.apiBaseUrl}/admin/organizations/${encodeURIComponent(orgId)}/members`,
     ...
     */
    async adminSetMemberRoles(orgId: string, userId: string, roleCodes: string[]): Promise<OrgMemberRecord> {
        const resp = await this.deps.ports.transport.request<OrgMemberRecord>({
            method: 'PUT',
        ...
        await ensureOk(resp);
        return resp.body as OrgMemberRecord;
    }
    ...
    async listRoles(orgId: string): Promise<OrgRoleRecord[]> {
        const resp = await this.deps.ports.transport.request<{ roles: OrgRoleRecord[] }>({
            method: 'GET',
    ...
    async getRole(orgId: string, roleId: string): Promise<OrgRoleRecord> {
        const resp = await this.deps.ports.transport.request<OrgRoleRecord>({
            method: 'GET',
    ...
    async listAssignablePermissions(orgId: string): Promise<AssignablePermissionRecord[]> {
        const resp = await this.deps.ports.transport.request<{ permissions: AssignablePermissionRecord[] }>({
            method: 'GET',
    ...
    async createInvitation(orgId: string, body: CreateInvitationRequest): Promise<InvitationRecord> {
        const resp = await this.deps.ports.transport.request<InvitationRecord>({
            method: 'POST',
    ...
    async listInvitations(orgId: string): Promise<InvitationRecord[]> {
        const resp = await this.deps.ports.transport.request<{ invitations: InvitationRecord[] }>({
            method: 'GET',
            url: `${this.deps.apiBaseUrl}/orgs/${encodeURIComponent(orgId)}/invitations`,
        });
        await ensureOk(resp);
     ...
     */
    async acceptMyInvitation(invitationId: string): Promise<Organization | null> {
        const resp = await this.deps.ports.transport.request<{ organization: Organization }>({
            method: 'POST',
    ...
    async declineMyInvitation(invitationId: string): Promise<void> {
        const resp = await this.deps.ports.transport.request({
            method: 'POST',
