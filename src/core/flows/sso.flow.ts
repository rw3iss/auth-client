...
export class SsoFlow {
    ...
    async start(req: SsoStartRequest): Promise<SsoStartResult> {
        ...
        if (req.organizationId !== undefined) body['organization_id'] = req.organizationId;
        if (req.inviteCode !== undefined) body['invite_code'] = req.inviteCode;
    ...
    async complete(req: SsoCompleteRequest): Promise<AuthResponse> {
        ...
        if (isAuthCodeResponse(callbackResp.body)) {
            ...
            if (!verifier) {
                throw new Error(
                    'PKCE verifier not found in storage — SSO start() must run in the same browser/profile',
                );
            }
            const exchangeResp = await this.deps.ports.transport.request<AuthResponse>({
                method: 'POST',
        ...
        await this.clearPersistedPkce();
        return callbackResp.body as AuthResponse;
    }

    private async clearPersistedPkce(): Promise<void> {
        await this.deps.ports.storage.remove(VERIFIER_STORAGE_KEY);
        await this.deps.ports.storage.remove(STATE_STORAGE_KEY);
    }
}

function isAuthCodeResponse(
    body: unknown,
): body is { auth_code: string; expires_in: number } {
    if (typeof body !== 'object' || body === null) return false;
    const b = body as { auth_code?: unknown };
