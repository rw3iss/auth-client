...
/* ------------------------------------------------------------------
 * Port interfaces. Each is implemented in core/adapters/ with a default
 * suitable for the browser. The constructor accepts overrides for SSR /
 * tests / custom storage backends.
 * ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------
 * Event types — emitted by AuthClient. Subscribers attach handlers via
 * client.on(event, handler).
 * ------------------------------------------------------------------ */

export type AuthEvent =
    | { type: 'authenticated'; user: User; tokens: TokenPair }
    | { type: 'logged_out'; reason: LogoutReason }
    | { type: 'token_refreshed'; tokens: TokenPair }
    | { type: 'requires_two_factor'; email: string }
    | { type: 'session_expired' }
    | { type: 'status_changed'; status: AuthStatus; snapshot: AuthSnapshot }
    | { type: 'org_switched'; organizationId: string; tokens: TokenPair }
...
export type AuthEventHandler<T extends AuthEventType = AuthEventType> = (
    event: Extract<AuthEvent, { type: T }>,
) => void;
