 ...
 */
export function secondsUntilExpiry(token: DecodedAccessToken, nowSeconds: number): number {
...
export function tryDecodeAccessToken(token: string): DecodedAccessToken | null {
    try {
...
function parseJsonSegment(segment: string, what: 'header' | 'payload'): Record<string, unknown> {
    let json: string;
    try {
