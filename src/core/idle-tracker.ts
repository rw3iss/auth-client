...
export class IdleTracker {
    private readonly opts: Required<IdleTrackerOptions>;
    ...
    stop(): void {
        ...
        this.started = false;
        for (const ev of ACTIVITY_EVENTS) {
            try { window.removeEventListener(ev, this.boundOnActivity, { capture: true } as EventListenerOptions); } catch { /* ignore */ }
        }
        try { document.removeEventListener('visibilitychange', this.boundOnVisibility); } catch { /* ignore */ }
    ...
    private recordActivity(): void {
        const now = Date.now();
        if (now - this.lastWriteMs < this.opts.throttleMs) return;
