...
export class WebCryptoAdapter implements Crypto {
    randomBytes(length: number): Uint8Array {
        const out = new Uint8Array(length);
        if (typeof globalThis.crypto?.getRandomValues !== 'function') {
            throw new Error(
                'WebCrypto getRandomValues is not available; supply a custom Crypto port',
            );
        }
    ...
    async sha256(input: Uint8Array): Promise<Uint8Array> {
        if (typeof globalThis.crypto?.subtle?.digest !== 'function') {
            throw new Error(
                'WebCrypto subtle.digest is not available; supply a custom Crypto port',
            );
        }
