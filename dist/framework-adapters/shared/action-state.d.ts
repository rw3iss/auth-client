...
export interface ActionState<T> {
    /** Result of the last successful invocation, or null. */
    data: T | null;
    /** Error from the last failed invocation, or null. */
    error: Error | null;
    /** True while a call is in flight. */
    loading: boolean;
    /** True when run() has never been invoked OR reset() was just called. */
    isIdle: boolean;
}
export interface Action<TArgs extends unknown[], TResult> extends ActionState<TResult> {
    /** Invoke the underlying flow. The returned promise mirrors the
     * native flow's promise so callers can `await action.run(...)`. */
    run: (...args: TArgs) => Promise<TResult>;
    /** Clear data + error; flip back to idle. Does not abort an
     * in-flight call. */
    reset: () => void;
}
/** Initial state — exported so adapters can use it as the seed for
 * their reactive primitive (useState / signal / ref). */
export declare const initialActionState: ActionState<unknown>;
/**
 ...
 */
export declare function runAction<TArgs extends unknown[], TResult>(op: (...args: TArgs) => Promise<TResult>, setState: (next: ActionState<TResult>) => void, args: TArgs): Promise<TResult>;
/**
 ...
 * at the call site.
 */
export declare function runOptimisticAction<TArgs extends unknown[], TResult, TSnapshot>(args: {
    op: (...a: TArgs) => Promise<TResult>;
    callArgs: TArgs;
    ...
    setState: (next: ActionState<TResult>) => void;
}): Promise<TResult>;
