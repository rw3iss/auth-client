...
export function fromHttpResponse(status, body) {
    ...
    if (status === 400)
        return new ValidationError(message, { serverCode });
    if (status === 401)
        return new UnauthenticatedError(message, { serverCode });
    if (status === 403)
        return new ForbiddenError(message, { serverCode });
    if (status === 404)
        return new NotFoundError(message, { serverCode });
    if (status === 409)
        return new ConflictError(message, { serverCode });
    if (status === 429)
        return new RateLimitedError(message, { serverCode });
    if (status >= 500)
        return new ServerError(message, { status, serverCode });
