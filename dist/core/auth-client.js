...
import { OrgFlow, } from './flows/org.flow.js';
import { AppsFlow, } from './flows/apps.flow.js';
import { M2MFlow, } from './flows/m2m.flow.js';
...
import { NamespacesFlow, } from './flows/namespaces.flow.js';
...
import { createDefaultBroadcast, NoOpBroadcast, } from './adapters/broadcast-channel.js';
...
export class AuthClient {
    ...
    constructor(config) {
        ...
        const ctx = {
            flows,
            guard: (operation) => this.assertOnline(operation),
            core: this,
        };
    ...
    async runBootstrap() {
        ...
        const hasValidClaims = this.cachedClaims !== null && !this.isExpired(this.cachedClaims);
    ...
    async authenticatedRequest(req) {
        ...
        const initial = await this.ports.transport.request(req);
        ...
        return this.ports.transport.request(req);
    }
    /* ------------------------------------------------------------------
    ...
    async getRegistrationPolicy(appCode) {
        ...
        if (!code) {
            throw new Error('getRegistrationPolicy requires an app code: pass one explicitly or set AuthClientConfig.appCode');
        }
     ...
     * override.
     */
    async requestMagicLink(email, appCode) {
     ...
     * Internal helpers
     * ------------------------------------------------------------------ */
    async persistAndAnnounce(result) {
        if (!result.tokens || !result.user)
            return;
        await this.persistTokens(result.tokens);
        this.transitionTo('authenticated');
    ...
    async getAccessTokenForTransport() {
